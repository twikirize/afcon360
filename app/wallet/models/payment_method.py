"""
app/wallet/models/payment_method.py

Payment Method Configuration - Single Source of Truth
All payment methods are defined and managed here.
Consumed by accommodation, events, and admin modules.
"""

from datetime import datetime, timezone
from sqlalchemy import Column, BigInteger, String, Boolean, DateTime, Text, JSON, ForeignKey, Numeric
...
class PaymentMethodConfig(BaseModel):
    ...
    method_id = Column(String(50), nullable=False, unique=True, index=True)
    ...
    def __init__(self, method_id, display_name, method_type, provider_name, country_code, **kwargs):
    ...
    def supports_currency(self, currency):
        return currency.upper() in [c.upper() for c in self.supported_currencies or []]

    def calculate_fee(self, amount):
        return float(amount) * float(self.transaction_fee or 0)
    ...
    def initialize_defaults(cls):
        defaults = [
            ...
            {
                'method_id': 'mock_gateway',
                'display_name': 'Test Gateway (Sandbox)',
                'method_type': 'test',
                'provider_name': 'afcon360_mock',
                'country_code': 'UG',
                'is_enabled': False,
                'is_active': False,
                'requires_phone': False,
...
class EventPaymentPreference(BaseModel):
    ...
    accepted_methods = Column(JSON, default=list)
    ...
    wallet_conversion_rate = Column(Numeric(10, 6), default=1.000000, nullable=False)
