...
class Aggregator(db.Model):
    ...
    aggregator_mode = Column('mode', String(20), nullable=False, default='testing', index=True)  # testing, live

    @property
    def mode(self):
        return self.aggregator_mode

    @mode.setter
    def mode(self, value):
        self.aggregator_mode = value

    sandbox_api_key = Column(String(255), nullable=True)
    sandbox_api_secret = Column(String(255), nullable=True)
    live_api_key = Column(String(255), nullable=True)
    ...
    def to_dict(self, exclude_secret=True):
        ...
        if not exclude_secret:
            ...
            data['sandbox_api_key'] = self.sandbox_api_key
            data['sandbox_api_secret'] = self.sandbox_api_secret
            data['live_api_key'] = self.live_api_key
            data['live_api_secret'] = self.live_api_secret
