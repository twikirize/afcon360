...
def my_registrations():
    """Redirect to attendee dashboard - legacy URL."""
    return redirect(url_for('events.attendee_dashboard'))
...
def register(identifier):
    ...
    try:
        ...
        if booking_type == BookingType.GROUP.value:
            group_booking_id = str(uuid.uuid4())
            logger.info("Created group_booking_id: %s", group_booking_id)
        
        group_label = data.get('group_label', '').strip() or None

        # Server-side group size cap (Phase 4.1)
        if booking_type == BookingType.GROUP.value:
            import json
        ...
        if booking_type == BookingType.GROUP.value:
            ...
            for idx, attendee in enumerate(group_attendees, start=1):
                ...
                attendee_nationality = attendee.get('nationality', '').strip()

                if not attendee_email or not attendee_name:
                    errors.append("Missing name or email for attendee")
                    continue
                ...
                try:
                    ...
                    attendee_data = {
                        ...
                        'nationality': attendee_nationality
                    }
                    if discount_code:
                    ...
                    if err:
                        ...
                        # Store reg and qr for the confirmation page
                        reg['qr_code'] = qr
                        registrations_created.append(reg)
                except SoldOutException as e:
                    errors.append(f"{attendee_name}: Ticket sold out - {str(e)}")
                    break
                except Exception as e:
                    errors.append(f"{attendee_name}: {str(e)}")
                    logger.error("Registration error for %s: %s", attendee_name, e)

            if registrations_created:
                flash(
                    f"Registered {len(registrations_created)} attendees. Errors: {len(errors)}",
                    'warning' if errors else 'success'
                )

                # Prepare session data for confirmation
                first_reg = registrations_created[0]
                session['last_registration'] = {
                    'registration': first_reg,
                    'qr_code': first_reg.get('qr_code'),
                ...
                if is_ajax:
                    return jsonify({
                        'success': True,
                        'registration_ref': first_reg['registration_ref'],
                        'redirect': url_for('events.registration_confirmation', reg_ref=first_reg['registration_ref']),
                        'errors': errors
                    })
                else:
                    return redirect(url_for('events.registration_confirmation', reg_ref=first_reg['registration_ref']))
            else:
        ...
        attendee_nationality = data.get('attendee_nationality', '').strip()
        ...
        if booking_type == BookingType.GROUP.value and view_model.is_paid_event and data.get('ticket_type_id'):
            import json
            from app.events.models import EventRegistration
            group_attendees_data = data.get('group_attendees_data', '[]')
            try:
            ...
            for i, attendee in enumerate(group_attendees, start=1):
                ...
                nationality = (attendee.get('nationality') or '').strip()
            ...
            for r in regs:
                ...
                if robj:
                    reg_dict = EventService._registration_to_dict(robj)
                    # Add QR code to each group member
                    reg_dict['qr_code'] = EventService._generate_qr_code(robj.qr_token, robj.registration_ref)
                    group_display.append(reg_dict)
        ...
        if is_paid_event and data.get('ticket_type_id'):
            ...
            if booking_type == BookingType.THIRD_PARTY.value:
                attendee_payload = [{
                    ...
                    'nationality': data.get('attendee_nationality') or data.get('nationality')
                }]
            ...
            # Sync individual attendee fields into 'data' dict for Service compatibility
            if booking_type == BookingType.THIRD_PARTY.value:
                data['full_name'] = attendee_name
                data['email'] = attendee_email
                data['phone'] = attendee_phone
                data['nationality'] = attendee_nationality
...
def registration_confirmation(reg_ref):
    ...
    try:
        ...
        # Permission check - allow attendee, booker, or organizer
        if not registration:
...
def approve_event(identifier):
    ...
    if not is_system_admin(current_user):
        return jsonify({'success': False, 'error': 'Only system administrators can approve events'}), 403
    ...
    try:
        ...
        return jsonify({'success': True, 'message': f'Event "{event_model.name}" approved successfully'})
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'error': str(e)}), 500
...
def delete_event(identifier):
    ...
    # Check permission
    ok, err = can_delete_event(current_user, event)
    if not ok:
        return jsonify({'success': False, 'error': err}), 403

    # Determine target status based on super admin
    target_status = EventStatus.DELETED if is_super_admin(current_user) else EventStatus.ARCHIVED
...
def admin_dashboard():
    ...
    pending_approval = Event.query.filter_by(status=EventStatus.PENDING_APPROVAL, is_deleted=False).order_by(
        Event.created_at.desc()).all()

    # Get recent events - order by status to show published and suspended first, then by creation date
    recent_events = Event.query.filter_by(is_deleted=False).order_by(Event.status.desc(),
                                                                     Event.created_at.desc()).limit(20).all()

    # Get category distribution
    categories = db.session.query(Event.category, func.count(Event.id)).filter_by(is_deleted=False).group_by(
        Event.category).all()
...
def admin_approve(identifier):
    """Admin approve event"""
    from app.events.permissions import can_approve_event
    event = EventService.get_event_model(identifier)
    if not event:
        return jsonify({'success': False, 'error': 'Event not found'}), 404
    ok, err = can_approve_event(current_user, event)
    if not ok:
        return jsonify({'success': False, 'error': err}), 403
...
def admin_reject(identifier):
    """Admin reject event"""
    from app.events.permissions import can_reject_event
    event = EventService.get_event_model(identifier)
    if not event:
        return jsonify({'success': False, 'error': 'Event not found'}), 404
    ok, err = can_reject_event(current_user, event)
    if not ok:
        return jsonify({'success': False, 'error': err}), 403
    data = request.get_json()
    if not data:
...
def admin_suspend(identifier):
    """Suspend an active/published/approved event."""
    # FIX: fetch event FIRST, THEN check permissions
    event = resolve_event(identifier)
    if not event or event.is_deleted:
        return jsonify({'success': False, 'error': 'Event not found'}), 404

    ok, err = can_suspend_event(current_user, event)
    if not ok:
        return jsonify({'success': False, 'error': err}), 403
    ...
    # Allow suspending published OR approved events
    allowed_statuses = (EventStatus.PUBLISHED.value, EventStatus.APPROVED.value if hasattr(EventStatus, 'APPROVED') else 'approved', 'published', 'approved')
    # Only allow suspending APPROVED or PUBLISHED events
    if event.status not in (EventStatus.APPROVED, EventStatus.PUBLISHED):
        return jsonify({'success': False, 'error': f'Cannot suspend an event with status "{event.status.value}"'}), 400
...
def admin_deactivate(identifier):
    ...
    # Use pause-level permission for deactivation semantics
    ok, err = can_pause_event(current_user, event)
    if not ok and not is_system_admin(current_user):
        return jsonify({'success': False, 'error': err or 'Only admins can deactivate events'}), 403
    ...
    # Disallow deactivating terminal states
    if event.status in (EventStatus.ARCHIVED, EventStatus.DELETED):
        return jsonify({'success': False, 'error': f'Event is already "{event.status.value}"'}), 400
...
def admin_takedown(identifier):
    ...
    ok, err = can_takedown_event(current_user, event)
    if not ok:
        return jsonify({'success': False, 'error': err}), 403
...
@events_bp.route("/admin/<identifier>/publish", methods=['POST'])
@login_required
def admin_publish(identifier):
    ...
    # Only allow publish from APPROVED
    if event.status != EventStatus.APPROVED:
        return jsonify({'success': False, 'error': f'Cannot publish event with status "{event.status.value}"'}), 400
    ...
    try:
        db.session.commit()
        logger.info(
            f"MODERATION | publish | event={event.slug} | admin={current_user.id} "
            f"| previous_status={previous_status}"
        )
        return jsonify({'success': True, 'message': f'Event "{event.name}" published'})
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'error': str(e)}), 500
...
def admin_events():
    ...
    if normalized == "all":
        ...
        current_filter = "all"
    elif normalized == "taken_down":
        # Special admin view: items that were taken down (archived due to policy)
        # Criteria: status=ARCHIVED, is_deleted=True, takedown_category is not NULL
        events = (
            Event.query
            .filter(
                Event.status == EventStatus.ARCHIVED,
        ...
        current_filter = "taken_down"
    else:
        # sanitize_status maps lowercase values ("approved") to the DB string
        sanitized_status = EventService.sanitize_status(normalized)
        events = Event.query.filter_by(
            status=sanitized_status, is_deleted=False
    ...
    # Build counts for tabs including a dedicated 'taken_down' bucket
    try:
...
def api_pending_events():
    ...
    try:
        pending_events = Event.query.filter_by(status=EventStatus.PENDING_APPROVAL).order_by(
            Event.created_at.desc()).all()
        events_data = []
...
@events_bp.route("/api/<slug>/check-registration")
@login_required
def api_check_registration(slug):
