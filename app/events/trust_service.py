...
class EventTrustService:
    ...
    def calculate_trust_level(user: User) -> TrustLevel:
        ...
        #
        # IMPORTANT:
        # `Event.created_by_id` is a standard audit column and is not reliably
        # populated by EventService.create_event().
        #
        # The creator identity that create_event() actually sets is:
        #   created_by_type        = CreatorType.INDIVIDUAL / ORGANIZATION / SYSTEM
        #   created_by_entity_id   = user.id / organization_id / 0
        #
        # Therefore successful event history must use
        # created_by_type + created_by_entity_id.
        if settings.enable_event_history_boost:
            successful_events = Event.query.filter_by(
                created_by_type=CreatorType.INDIVIDUAL.value,
