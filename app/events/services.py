...
def sanitize_status(status_value):
    """
    Safely convert any status value to a valid EventStatus string (lowercase).

    Accepts:
      • EventStatus enum member  →  returns .value   ("approved")
      • lowercase string         →  validates & returns as-is  ("approved")
      • uppercase string         →  matches by enum name       ("APPROVED" → "approved")
      • "EventStatus.APPROVED"   →  this is a CODE BUG in the caller; we log
                                    an error and attempt recovery rather than
                                    silently returning DRAFT.
    """
    from app.events.constants import EventStatus
    ...
    if status_str.startswith("EventStatus."):
        enum_name = status_str.split(".", 1)[1]  # "APPROVED"
        logger.error(
            f"sanitize_status received a Python enum repr '{status_str}'. "
            f"The caller should use EventStatus.X.value, not str(EventStatus.X). "
            f"Attempting recovery via enum name '{enum_name}'."
        )
        try:
    ...
    # Try matching by enum name (e.g. "APPROVED" → EventStatus.APPROVED)
    try:
...
def _assert_no_open_flags(event_id: int):
    """Raise ValueError if the event has any unresolved ContentFlag records."""
    ...
    if count:
        raise ValueError(
            f"Cannot publish event {event_id}: {count} open moderation flag(s) must be resolved first."
        )
...
class EventService:
    ...
    def change_event_status(cls, event_id: str, new_status: str, user_id: int,
        ...
        if new_status in (EventStatus.APPROVED, EventStatus.REJECTED):
            # APPROVED / REJECTED require events.approve
            ...
            # SUSPENDED / DELETED require content.moderate
            ...
            # PUBLISHED requires events.approve OR being the event organizer
        ...
        # Other transitions (PAUSED, CANCELLED, ARCHIVED) are allowed for any authenticated user
        # (they are typically initiated by the organizer or admin)

        # Guard: cannot publish if there are open ContentFlag records
        ...
        try:
            db.session.commit()
            logger.info(
                f"Event {event.slug} status changed from {old_status.value} to {new_status.value} by user {user_id}")
            return True, None
        except Exception as e:
            db.session.rollback()
    ...
    @classmethod
    def create_event(cls, data: Dict, user_id: int, creator_type: str = 'individual', organization_id: int = None) -> \
    Tuple[Optional[Dict], Optional[str]]:
        """Create a new event with authorization checks"""
        try:
            from app.auth.helpers import has_global_role, has_org_role, has_global_permission
            ...
            # After saving, if pending approval, submit to moderation pipeline to create a ContentFlag
            try:
                ...
                # Do not fail event creation if moderation flagging fails; log for follow-up
    ...
    def _event_to_dict(cls, event: Event) -> Dict:
        ...
        return {
            ...
            # Still exists in model, but primary fee is on ticket_type
    ...
    def register_for_event_optimistic(cls, identifier: str, user_id: int, data: Dict,
        ...
        if idempotency_key:
            if IdempotencyChecker.check_and_store(idempotency_key):
                ...
                if existing_reg:
                    ...
                    return cls._registration_to_dict(
                        existing_reg), qr_code, "Duplicate request - registration already processed"
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 5. Get proper sequence number
                reg_count = db.session.query(func.count(EventRegistration.id)).filter_by(
                    event_id=event.id
                ).scalar() or 0
                ...
                # Generate refs with the correct sequence
    ...
    def add_to_waitlist(cls, identifier: str, user_id: int, data: Dict) -> Tuple[Optional[Dict], Optional[str]]:
        ...
        try:
            ...
            logger.info(
                f"User {user_id} added to waitlist for event {identifier} at position {waitlist_entry.position}")
    ...
    def add_ticket_type(cls, identifier: str, data: Dict, user_id: int) -> Tuple[Optional[Dict], Optional[str]]:
        ...
        try:
            ticket_type = TicketType(
                ...
                available_from=datetime.strptime(data["available_from"], "%Y-%m-%dT%H:%M") if data.get(
                    "available_from") else None,
                available_until=datetime.strptime(data["available_until"], "%Y-%m-%dT%H:%M") if data.get(
    ...
    def get_attendee_dashboard_data(cls, user_id: int) -> Dict:
        ...
        upcoming_registrations.sort(
            key=lambda x: x['event']['start_date'] if x['event']['start_date'] else '9999-12-31')
        past_registrations.sort(key=lambda x: x['event']['start_date'] if x['event']['start_date'] else '0000-01-01',
                                reverse=True)
    ...
    def get_organizer_dashboard_data(cls, user_id: int) -> Dict:
        ...
        # Get actual Event models for counting registrations
        ...
        for event_model in event_models:
            ...
            reg_count = EventRegistration.query.filter_by(event_id=event_model.id).count()
            reg_revenue = db.session.query(func.sum(EventRegistration.registration_fee)).filter_by(
    ...
    @classmethod
    def validate_discount_code(cls, identifier: str, code: str, ticket_type_id: Optional[int] = None) -> Tuple[
        Optional[Decimal], Optional[str]]:
        """
    ...
    def assign_service_to_attendee(cls, attendee_id: int, identifier: str,
        ...
        try:
            ...
            logger.info(
                f"Service assigned: {booking_type} booking {booking_id} to attendee {attendee_id} for event {identifier}")
