...
def assignment_dashboard(event_id):
    ...
    stats = {
        ...
        'accommodation_coverage': round((accommodation_assigned / total_attendees * 100), 1) if total_attendees > 0 else 0,
...
def check_available_properties(event_id):
    ...
    for prop in properties:
        is_available, _, _ = AvailabilityService.is_range_available(
            ...
            check_out=event.end_date.date() if event.end_date else event.start_date.date() if event.start_date else None
        )
