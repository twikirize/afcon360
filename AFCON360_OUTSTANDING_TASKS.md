...
|---|------|--------|----------|
| 11 | Role lists (Tier 1) | VERIFIED — already correct, no change | routes.py `view_role_list()` scopes by active persona (`primary_role` / `active_org_role`), not permissions |
| 3 | Capacity+copy (Tier 2) | VERIFIED — already correct, no change | `active_guests` counts placeholder/needs_data; capacity guard in place; DB `is_active`/`is_placeholder` rows confirmed; route delegates to `SpecialRequestService` |
| 10 | Placeholders (Tier 2) | IMPLEMENTED | checkout POST path keeps `send_notification` default `False`; reserve/create now mandatory; Status callback handles `confirmed` already-complete; ends with `; return redirect(...)` guard |
| 12 | Slot expiration (Tier 3) | VERIFIED | reminder sends only to `needs_data`; expiry growth bounded ≤ `INT_MAX/2`; no expiry rewrite path |
| 8 | edit_registration (Tier 2) | IMPLEMENTED | route + `guest/edit_registration.html`; roster edit link uses it; Jinja parse OK, endpoint validation passed |
| 1 | Defer toggle (Tier 3) | IMPLEMENTED | `guest_defer_registration` route + `guest/guest_roster.html` toggle; autogenerated `accommodation_guest_registrations` supports it |
| 6 | Delegation (Tier 3) | IMPLEMENTED | `guest_delegate_registration` route + roster delegation UI + `guest_roster.js`; `auth_delegations` table present, `requires_approval` default `false` |
| 7 | Single-guest add (Tier 2) | IMPLEMENTED | `guest_add_guest` routes (GET/POST) + `guest/guest_roster.html` count-aware add panel |
| 2 | FX note (do-next) | IMPLEMENTED | `checkout.html` `data-currency` + `#fxNote`; `checkout.js` `showFxNote()` via verified `GET /api/fx/rates/<base>/<quote>`; JS syntax OK |
| 13 | Request types (do-next) | IMPLEMENTED | column already existed (`booking_policy.py:94`); host config UI present; wired `guest_add_request` route + `add_request.html` options loop w/ fallback |
| 14 | Preferred currency (do-next) | VERIFIED — already wired, no change | `PropertyPaymentMethod.preferred_currency` (property_payment_method.py:38) read by `get_allowed_options()` |
| 15 | Billing details (do-next) | IMPLEMENTED | standalone `.price-item/.total/.amount/.payment-label` CSS added in `checkout.css` |
| 5 | Pre-arrival reminder (scheduled) | IMPLEMENTED | `accommodation.send_pre_arrival_request_reminder` + beat entry; uses real `NotificationType.BOOKING_UPDATE` + explicit module |
| 17 | Row-lock concurrency (scheduled) | IMPLEMENTED + VERIFIED | `tests/test_registration_row_lock_concurrency.py` — `2 passed`; 4 ok + 1 rejected, no deadlock, sequential < 15s |
| 16 | auth_delegations migration | VERIFIED — applied | DB stamped at single head `1788729103`; `auth_delegations` table EXISTS; `requires_approval` default `false` |
| 18 | Addendum migration | VERIFIED — applied | `accommodation_booking_special_requests`, `accommodation_booking_registration_links`, `accommodation_guest_registrations` + all slot-lifecycle columns present |
| 4 | RoomHold design review | STOPPED — discrepancy found | `create_hold()` DOES instantiate `RoomHold` (availability_service.py:484) and uses `InventoryBlock` for multi-unit — premise stale; design session required |

**Discrepancies / deferred (see BACKLOG.md):** Task 4 design review untriaged; Task 5 exposed a latent bug — legacy `registration_reminder_*` notification types are not in the `NotificationType` enum so `NotificationService.send` silently drops them; Task 3 did not add a route-level regression test as its step 3 requested.

**Verification performed (2026-09-07):** `flask db current/heads` → single head `1788729103`; table/column inspection against live DB → all present; Jinja parse OK on 5 edited templates; `node --check` OK on `checkout.js` + `guest_roster.js`; `pytest` on the three registration test files → 14 passed (flow 9 + addendum 3 + concurrency 2), no regressions.
