...
#########################################
This prompt is well-constructed — the four findings you've echoed back are accurate, matches exactly what I flagged, and grounding an agent with "acknowledge before you code" is a good technique for avoiding regressions. It'll fix Tasks 1, 2, and most of 4. But there are three gaps that could let this session repeat the same "reported done, wasn't" pattern from before — worth patching the prompt before you send it.
1. Task 3 depends on a file you haven't included, and haven't verified.
get_property_calendar_snapshot will only be accurate if AccommodationBooking.room_type_id is actually populated when a guest books. That happens in whatever route/service handles booking creation — which isn't in your target file list. If that route still only sets property_id (unverified — we never confirmed it), Task 3's refactor will run without errors but silently show every room as fully available, which is worse than the current bug because it looks correct. Add that file to the target list, or add an explicit Task 0: "Locate the booking-creation code path and confirm/set room_type_id on every new AccommodationBooking before touching the calendar logic."
2. Task 3 doesn't specify what happens for a property with multiple RoomTypes.
The current function returns one day-by-day status per property_id — fine for a 1-room host, meaningless for a hotel with Deluxe/Suite/Standard each having different availability on the same date. Right now the prompt leaves this undefined, which means Gemini will guess — likely either silently ignoring room_type_id (reverting to the old bug) or picking a shape you didn't ask for. Add: "If a property has more than one RoomType, the snapshot must be scoped per room_type_id (accept it as an optional parameter) — do not collapse multiple room types into a single property-level status."
3. Task 4 — don't let it edit the already-applied migration file.
You listed 1d30290f4f67_add_room_types_and_inventory_blocks_.py as a target file. That migration has already run against your database — editing it directly won't change anything in the actual DB (Alembic tracks it as already-applied and won't re-run it), so InventoryBlock.reason in the model would say db.Enum while the real Postgres column stays VARCHAR(30). That's the exact "model says one thing, schema says another" mismatch that caused your original crash. Add: "Task 4 requires a NEW scoped migration (flask db migrate) — do not modify the already-applied 1d30290f4f67 file."
One judgment call worth surfacing, not necessarily fixing: your own media_handling.md documents "no sa.Enum types — avoids PostgreSQL ENUM conflicts" as a deliberate project rule (native Postgres ENUMs are painful to extend later — adding a new block reason means an ALTER TYPE migration, which is more ceremony than adding a string). property.py already contradicts that rule elsewhere for other status fields, so it's not unprecedented, but block reasons specifically (maintenance, renovation, etc.) are the kind of list that tends to grow. A CheckConstraint on the existing String(30) gets you the same DB-level enforcement without locking into native ENUM migration overhead. Not saying don't do it as written — just make it a deliberate choice rather than something that happens because it was in the prompt.
Add one more line regardless of the above: "Confirm each task by actually running the app (flask run) and exercising the affected code path — not just confirming the code imports cleanly — before reporting any task complete." Worth restating explicitly since Gemini has no memory of the pattern established with Kilo in this thread (claiming "✅ Complete" for things that were never executed, three times running).
With those four additions, I'd trust this to actually close the gap rather than just move it.