...
def test_host_onboarding_no_longer_creates_property(client, verified_user, app):
    ...
    Onboarding is a single identity step: it records the provider
    intention and routes to the host dashboard."""
    ...
    assert r1.status_code == 302
    assert "host" in r1.headers["Location"].lower()
...
def test_host_onboarding_legacy_create_property_flag_still_works(app, verified_user):
