  ...
  const Dashboard = {
    ...
    cacheElements() {
      this.els = {
        ...
        pendingTable:    document.getElementById('pendingSubmissionsTable'),
      };
    },
    ...
    async refreshAllData() {
      try {
        const [perf, queue, audit] = await Promise.all([
          this.fetchJSON('/moderator/performance'),
          this.fetchJSON('/moderator/queue_metrics'),
          this.fetchJSON('/moderator/audit_insights'),
        ]);
        ...
        // Also refresh My Queue panel if it exists
        if (typeof loadMyQueue === 'function') {
          loadMyQueue();
        }
        // Refresh pending submissions table
    ...
    renderQueue(data) {
      ...
      html += `<tr><td>Assigned</td><td><span class="badge b-normal">${assigned}</span></td></tr>`;
      ...
      html += `<tr><td>Avg Processing Time</td><td><span class="badge b-normal">${avgTime}s</span></td></tr>`;
      html += `</tbody></table>`;
      container.innerHTML = html;
    },
    ...
    // ──────────────────────────────────────────────────────────────
    // REFRESH PENDING TABLE (Phase 3)
    // ──────────────────────────────────────────────────────────────

    async refreshPendingTable() {
      const table = this.els.pendingTable;
      if (!table) return;

      try {
        const response = await fetch('/moderator/content?status=pending&per_page=10&partial=1', {
          headers: { 'Accept': 'text/html' },
          credentials: 'same-origin'
        });
        if (!response.ok) return;
        const html = await response.text();
        // Extract the tbody from the response
        ...
        // Update cached rows reference
        this.els.submissionRows = document.querySelectorAll('#pendingSubmissionsTable tbody tr');
        // Re-apply SLA indicators to new rows
        this.applySLAIndicators();
      } catch (e) {
        console.warn('Failed to refresh pending table:', e);
      }
    },
  };
