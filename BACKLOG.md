...
- **Verification:** All 38 tests pass across `tests/transport/test_reservation_d3.py` and `tests/transport/test_reservation_expiry.py` (including sequential overlap/release tests and two real PostgreSQL concurrency tests: `test_same_vehicle_same_window_one_wins` — exactly one winner + one `ReservationConflictError`; `test_capacity_race_60_plus_60_on_100` — exactly one success + one conflict on 100-unit supply). `flask db current == flask db heads == 5e80dc4ac345` (single head). No migration required — removed ExcludeConstraints and `btree_gist` listeners that were never materialised in the live DB; model metadata verified via `scripts/setup_test_db_schema.py`. `STARTUP_OK`. No new Alembic revision needed.
- **Deferred:** D5 materialisation seam remains intentionally untouched. Scheduling this expiry primitive remains deferred; Batch 2 creates no beat task.
...
- **Reconciliation (2026-09-13 — G-1 PRODUCTION REACHABILITY CLOSURE, verdict PASS):** The 2026-09-06 "DONE" record above was only DEFENSIVE, not operational. The two-gate (route + service write-boundary) was added to the org branch, but the org branch itself was UNREACHABLE via the real entry point: `_ensure_host_identity` (`app/accommodation/routes.py:4164`) called `get_host_identity(current_user)` with no `org_id`, so `host_info["type"]` was ALWAYS `"individual"` → every org host context aborted 403 and the route's org two-gate lines were dead code. The 2026-09-06 route tests used `_fake_host_identity(...)` monkeypatches (org_id_ kwarg — a signature the real service does not accept), so they never exercised the production path. **PROVEN DEFECT (this node, RED):** an eligible, capability-ACTIVATED, `OrganisationMember`-backed org host hitting `host_create_listing` got `abort(403)` before the two-gate could run; the org-property write matrix positive was confined to monkeypatched tests. **MINIMAL FIX (production-only, routes.py):** in `_ensure_host_identity`, when `active_context.type is ACCOMMODATION_HOST` and `str(active_context.public_id) != str(current_user.public_id)`, resolve the `Organisation` by public `org_id` (abort 403 if missing) and call `get_host_identity(current_user, org_id=organisation.id)`; org branch retained. All existing flows (individual host, session sanitization to PERSONAL, 403-ineligible) unchanged; service write-boundary two-gate untouched (`host_service.create_property` already gated). **Test alignment:** two legacy fakes in `tests/test_stage4b6_org_write_gate.py` renamed kwarg `org_id_=None` → `org_id=None` to mirror the real service signature — test-double alignment only, proves the route now reaches them. **NEW unpatched production-path suite `tests/test_org_host_production_path.py` -> 6 passed** (no monkeypatching): (1) org branch GET create 200; (2) POST create 302 → `Property` persisted with `owner_org_id` set, `owner_user_id is None`; (3) foreign-org context sanitized to PERSONAL → 302 + no write; (4) `org_admin` member (lacks `org.accommodation.manage`) → 403; (5) INTENT capability → 302 to `/host/dashboard`, 0 properties persisted; (6) RESTAURANT ineligible org → 302, 0 properties. Context semantics verified: `get_active_context` only offers an org host context when `can_org_host(org_id)` is True, so foreign/ineligible orgs are sanitized to PERSONAL and redirect; in-context members lacking the manage permission hit the routes.py abort(403). **VERIFY:** `test_stage4b6_org_write_gate.py` + `test_org_host_production_path.py` + `test_can_org_host.py` + `test_org10_adversarial_cross_property.py` -> **76 passed**; `test_stage4b2_capability_enforcement.py` -> **33 passed**; added `test_auth_context.py` -> combined **121 passed**. Migration head `a91ce8028ae4 (head)` unchanged; no migration; & force: org hosting still gated on individual `can_host()` first (owner/super_admin bypass KYC; else full KYC + profile) — recorded as a residual observation, NOT part of this defect. G-1 GATE: **PASS**. NEXT AUTHORIZED NODE (awaiting user instruction): Stage 4B-7 re-run to fold this closure into the Full Stage-4B Verification baseline.
...
## Stage 4B-7 Re-run — Full Stage-4B Verification Gate RE-CONFIRMED PASS (2026-09-13, post-G-1/EGGE)
- **Status:** DONE (2026-09-13) — forensics/verification only; no implementation, no migration, no repair of pre-existing drift; Stage 5 still authorized next (Stage 5 NOT started by this node)
- **Raised:** 2026-09-13
- **Context:** Authorized re-run of the closed Stage 4B-7 gate (prior PASS 2026-09-06 at BACKLOG.md:875) with the closure deltas in scope: G-1 org-write-gate minimal fix (`app/accommodation/routes.py` 4377-4402 + `host_service.py:96-118` write-boundary invariant) and the EGGE identity/context audit (auth/context.py DB-backed fail-closed descriptors, helpers, org routes, OrganizationPermissionService). Task: re-verify the full Stage 4B chain — onboarding → org → Provider Participation → membership → org role → org context → capability → permission → accommodation eligibility → ownership → operational write → cross-user/cross-org isolation — and issue the gate. Closed gates (4B-6/G-1, identity/context audit) were NOT reopened absent direct contradictory evidence; no architecture redesign; no migration authority used.
- **PROVE (all mandated + regression batches green, 0 failed):** (1) 4B-1 onboarding+org RBAC: `test_onboarding_stage4.py` (28) + `test_org_creation_rbac_4b1.py` (13) -> **41 passed** (~74s). (2) 4B-2 capability enforcement + 4B-3 org capabilities: `test_stage4b2_capability_enforcement.py` + `test_stage4b3_organisation_capabilities.py` -> **49 passed** (~88s). (3) eligibility + capability ops + PP: `test_can_org_host.py` (45) + `test_capability_operations.py` + `test_provider_participation.py` -> **121 passed** (~115s). (4) 4B-5 transport (7) + 4B-6/G-1 write gate (11) + org host production path (6) -> **24 passed** (~89s). (5) membership/role/context (EGGE basis): `test_auth_context.py` + `test_assign_revoke_org_role.py` + `test_organisation_role_provisioning.py` + `test_org_workspace.py` + `test_org_permission_read_path.py` -> **58 passed** (~77s). (6) directed regression on write-gate surfaces: `test_onboarding_new.py` + `test_accommodation_roomtype.py` + `test_can_org_host.py` -> **79 passed** (~126s). **Total re-run evidence: 372 passed, 0 failed.** Startup sanity `python -c "from app import create_app"` -> exit 0, `STARTUP_OK`.
- **TRACE conclusion (production paths A–E, consistent with prior PASS + gates):** (A) onboarding commit persists creator as org owner via per-org `OrgRole` provisioning (roles.py:241), no legacy FK mismatch; (B) human login authenticates users only (services.py:435); (C) org context resolution DB-backed and fail-closed (context.py validate/switch/get_active/resolve_effective_permissions), permissions via helpers + OrganizationPermissionService; (D) accommodation org write gated by BOTH `can_org_host` AND `is_capability_operational(...,ACCOMMODATION)` (route) plus `HostService.create_property` write-boundary invariant (defense in depth) — verified by 4B-6 (11) + production-path (6); (E) cross-user/cross-org isolation proven by write-gate cross-authority test + EGGE context suites. No production OPC writers; PP single universal participation authority; `can_org_host` uses canonical `OrganizationType` eligibility (Option C, business-category reconciliation held).
- **DB / migration proof:** `flask db current` -> `a91ce8028ae4`; `flask db heads` -> `30ad1d810b0b (head)` (single head). **Pre-existing drift (reported, NOT repaired):** the unapplied transport-reservation migration `30ad1d810b0b_transport_reservation_obligation_and_.py` (untracked) is the head, DB is one behind — same drift recorded by prior transport nodes (was `f91b7a92209b`, regenerated to `30ad1d810b0b` by the TH-3-D3 Batch 2 node); no Stage-4B schema dependency, no NEW drift introduced by this node. Working tree additionally shows `migrations/env.py`, `.env.test`, `.env.testing`, `app/transport/services/reservation_service.py`, transport test files as modified, plus `scripts/_probe_migrate.py` + `scripts/_probe_reflection.py` (probe artifacts from the transport node) — all pre-existing relative to this node; `app/accommodation/routes.py` + `tests/test_stage4b6_org_write_gate.py` (G-1) and `tests/test_org_host_production_path.py` (EGGE) are the authorized closure-delta files.
- **GATE:** **PASS (re-confirmed)**. No failure, defect, or contradiction surfaced against the closed G-1 or identity/context gates. Stage 4B written invariants all hold on the current codebase.
- **NEXT AUTHORIZED NODE:** Stage 5 — Cross-Domain Integration (unchanged from prior PASS). NOT started by this node.
- **Deferred (unchanged, do not silently fix):** transport REST admin-provisioning legacy endpoints (prior 4B-7 bypass item, non-blocking); transport reservation migration `30ad1d810b0b` unapplied (operator executes per §20); `scripts/setup_test_db_schema.py` hang + `scripts/_probe_*` cleanup (transport infra); M-series transport findings (M1-M5, TH-3 audits); prior Stage 5-1..5-4 deferred families.
- **Links:** tests/test_onboarding_stage4.py, tests/test_org_creation_rbac_4b1.py, tests/test_stage4b2_capability_enforcement.py, tests/test_stage4b3_organisation_capabilities.py, tests/test_can_org_host.py, tests/test_capability_operations.py, tests/test_provider_participation.py, tests/test_stage4b5_transport.py, tests/test_stage4b6_org_write_gate.py, tests/test_org_host_production_path.py, tests/test_auth_context.py, tests/test_assign_revoke_org_role.py, tests/test_organisation_role_provisioning.py, tests/test_org_workspace.py, tests/test_org_permission_read_path.py, tests/test_onboarding_new.py, tests/test_accommodation_roomtype.py, app/accommodation/routes.py:4377-4402, app/accommodation/services/host_service.py:96-118, app/auth/context.py, app/auth/services.py:435, app/auth/roles.py:241, app/identity/services/organization_permissions.py, migrations/versions/30ad1d810b0b_transport_reservation_obligation_and_.py, BACKLOG.md:875 (prior 4B-7)
...
## Org RBAC + Membership + Administration Functional Audit (READ-ONLY)

- **Status:** COMPLETED (audit delivered 2026-09-13)
- **Resolved:** 2026-09-13
- **Nature:** AUDIT/PROOF only � no code, test, migration, or schema changes made
- **Scope:** Who controls an org; member add + verification; role assign/revoke; role?permission mapping; org dashboard functionality; safe permission exercise in correct org context
- **Evidence inspected:** app/identity/routes.py (org_bp), app/identity/services/organization_permissions.py, organization_registration.py (add_org_member), app/identity/models/organisation_member.py + organization_types.py, app/auth/seed_roles.py (ORG_PERMISSION_DEFS / ORG_ROLE_TEMPLATES), app/auth/roles.py, app/auth/helpers.py, app/auth/context.py, app/auth/decorators.py, app/auth/services/org.py, app/admin/route_modules/org_admin.py, templates/org/*.html
- **Tests run (in-session, sequential, shared afcon360_test):** 116 passed / 0 failed � test_org_workspace (4), test_assign_revoke_org_role + test_organisation_role_provisioning + test_org_permission_read_path (42), test_org_creation_rbac_4b1 + test_auth_context + test_org_provider_capability + test_onboarding_stage4 (70)
- **DB read-only proof (live test DB):** canonical org.* Permission rows exist (15); ALL legacy gate names MISSING � org.manage_staff, org.manage_settings, org.create_events, org.manage_accommodation, org.manage_transport, org.manage_tourism, org.manage_wallet, org.view_reports, org.full_control
- **Key findings:**
  1. CONTRADICTION � RBAC AUTHORITIES DISAGREE: org dashboard routes gate on legacy permission names that are NEVER seeded; every member page (members/settings/events/accommodation/transport/tourism/wallet/reports) is therefore unreachable for ALL roles including org_owner (fail-closed but broken UI)
  2. GAP � DASHBOARD PATH NOT PROVEN: /org/<id>/members gated on org.manage_staff (MISSING); even if opened, get_organization_hierarchy uses member.role (not a column/property) and members.html uses member.role.value (member not passed) ? AttributeError / UndefinedError 500
  3. GAP � BROKEN MEMBER LIFECYCLE: only remote-member route is admin org_admin_remove_member, which imports non-existent app.organization ? ImportError swallowed ? dead; no functional remove/leave/revoke-role HTTP path exists
  4. GAP � MEMBER VERIFICATION NOT PROVEN: add path = add_org_member(existing-user-by-email, is_active=True, no email/SMS verify); send_invite is a TODO; no invite/request/approve lifecycle (admin approve/reject dead)
  5. DEFECT � PRIVILEGE ESCALATION (LATENT): add_org_member accepts ANY role_name incl. org_owner/org_admin with no guard; intended guard can_add_member/can_change_role/can_remove_member has ZERO callers; safe only because the gate is currently closed
  6. CONTRADICTION � ROLE VOCABULARY MISMATCH: UI offers legacy OrganizationRole enum (16 values: org_manager, staff_member, viewer, ...) but add_org_member resolves OrgRole by name against the 9 provisioned ORG_ROLE_TEMPLATES ? most offered roles raise ValueError on submit
  7. DEAD CODE: transfer_org_ownership (app/auth/services/org.py) queries nonexistent OrgUserRole columns; unreferenced
  8. PASS � IMPLEMENTED AND PROVEN: org creation?org_owner OrgUserRole; idempotent per-org role provisioning; assign/revoke_org_role (per-org OrgRole, no global mutation); permission read path (role perms + direct grant/deny + org isolation + capability?authority); DB-backed fail-closed org context; public-id org URLs; POST context switch in base nav
- **Recommended next node:** Org Administration Remediation Node (wire role assign/revoke + member add/remove to canonical ORG_ROLE_TEMPLATES perms org.members.manage / org.members.manage_roles; add org_owner protection + escalation guard; fix members/settings dashboard paths) � requires SPECIFICATION first (�6), then implementation, add HTTP-path tests
- **Links:** app/identity/services/organization_permissions.py, app/identity/routes.py, app/identity/services/organization_registration.py, app/auth/seed_roles.py, app/admin/route_modules/org_admin.py (dead), app/auth/services/org.py (dead), templates/org/members.html

## Stage 4B-8/USER-A1 - Org Administration Remediation Implementation (HTTP paths) + Spec Correction
- **Status:** DONE (implementation node, resolved 2026-09-14)
- **Raised:** 2026-09-13 (audit recommendation at prior Org RBAC audit entry)
- **Context:** Follow-on to the Org RBAC + Membership + Administration Functional Audit (prior entry, 2026-09-13). Auizhorized Implementation node: re-map org admin routes from legacy (never-seeded) gate names to canonical `org.*` permissions, wire member lifecycle HTTP paths (add/remove/role-change) to `org.members.manage` / `org.members.manage_roles` with owner protection + escalation guard, fix members/settings/dashboard/transport templates, add HTTP-path test suite. SPECIFICATION first (pursuant to 444 MIGRATION sections); user correction (supersedes spec draft): `org.members.manage_roles` is held by BOTH `org_owner` AND `org_admin` (proven at `app/auth/seed_roles.py:216-217`); spec corrected accordingly.
- **Files changed (production):** `app/identity/routes.py` (route gates re-mapped to canonical perms; transport query fixed to `owner_type/owner_id` pattern; settings route now supplies `stats`; org selector computes owned/verified counts), `templates/org/members.html`, `templates/org/dashboard.html`, `templates/org/settings.html`, `templates/org/selector.html`, `templates/org/content_dashboard.html` (null-safe `org.business_category.value`), `templates/org/transport.html` (NEW - template was missing entirely), `app/identity/ORG_ADMINISTRATION_SPEC.md` (NEW spec artifact; manage_roles wording = org_owner + org_admin).
- **Files changed (tests):** `tests/test_org_admin_remediation_http.py` (NEW - scalar-handle HTTP suite, **30 passed / 0 failed**), `tests/conftest.py` (canonical `_login_client` merge pattern at 423-438 - prevents DetachedInstanceError on session teardown).
- **Behavior change:** org dashboard/members/settings/accommodation/transport pages now reachable for authorized members and no longer 500. Root causes fixed: (1) template `UndefinedError` on `org.business_category.value` when `business_category is None` (5 templates); (2) `Vehicle.operator_id` InvalidRequestError in transport route; (3) `url_for('org.kyb')` BuildError in dashboard template -> corrected `org.org_kyb`; (4) settings route missing `stats` context; (5) `memberships|selectattr('role.name',...)` invalid (OrganisationMember has no `.role`) -> route computes counts.
- **Authority map applied:** members GET = `org.members.view`; add/remove = `org.members.manage`; role administration = `org.members.manage_roles` (org_owner + org_admin); settings = `org.settings.manage`; accommodation = `org.accommodation.manage`; transport = `org.transport.manage`; events/tourism/wallet/reports = fail-closed (no canonical perms). Only the 9 canonical ORG_ROLE_TEMPLATES are assignable; org_owner escalation guard + owner immutability + last-owner protection enforced; self-role-change denied; non-owner self-leave allowed.
- **Verification:** `pytest tests/test_org_admin_remediation_http.py` -> **30 passed**; 9 Stage-4B/org suites (assign_revoke, 4b1, 4b2, 4b3, 4b5 transport, 4b6, can_org_host, org_provider_capability, onboarding_stage4) -> **186 passed / 0 failed**; `test_org_host_production_path.py` + `test_accommodation_checkout_processes.py` -> **16 passed**; startup `python -c "from app import create_app"` -> `IMPORT_OK`; `flask db current` == `flask db heads` == `a91ce8028ae4` (single head). NO migration created or applied (schema untouched).
- **GATE:** **PASS** (per 444 Full Stage-4B gate format: startup OK, current==heads==a91ce8028ae4, mandated + regression suites green, no failure/defect/contradiction, migration evidence recorded).
- **Deferred (do not silently fix):** `verify_db.py` on disk is a stale `datetime.utcnow()`-fixer script (NOT the DB verifier described in 444/436; prints "Fixed N files", no `DB_VERIFY_OK`) - pre-existing tree inconsistency; `scripts/setup_test_db_schema.py` hang (known); transport reservation migration drift recorded by prior transport nodes; dead `templates/org/members_old.html`; `.env.test`/`.env.testing`/`app/accommodation/routes.py`/`app/transport/services/reservation_service.py` working-tree mods are pre-existing (prior nodes/concurrent transport lane), not this node.
- **Reconciliation (2026-09-14, todo-list verification):** full 12-item Stage 4B to-do list verified fulfilled with evidence. Confirmed: (1) TRACE - forms (`app/forms/organization_forms.py` canonical `role_choices`, no `send_invite`), roles service (`app/auth/roles.py:241` `assign_org_role`), models (`organisation_member.py` canonical `OrgUserRole` graph, no `member.role` column), routes, templates; (3) `add_org_member` `assigned_by` routed into `OrgUserRole` (actor pk, audit); (5) `get_accessible_modules`/`get_organization_hierarchy` read canonical `member.roles`→`OrgRole.name` only (legacy `member.role` getattr fallbacks remain-but-unreachable); (6) gates re-mapped (members GET POST :294, role POST :391, remove POST :474, settings :543, accommodation :721, transport :762); (7) + (9) HTTP suite groups A-E at `tests/test_org_admin_remediation_http.py` (24 cases: view gates, add w/ assigned_by audit, dup/inactive/unverified/legacy-role rejection, no-escalation, owner grant, immutability, self-denial, remove purge, self-leave, fail-closed module gates, template no-legacy-enum); (11) spec `manage_roles` = org_owner+org_admin (ORG_ADMINISTRATION_SPEC.md:67,151,210). DEFECT FOUND DURING PROOF (authorized fix): real POST /login stored raw `OrganisationMember` ORM in `session["available_orgs"]` → Flask-Session Redis serializer `TypeError`; fixed at `app/auth/routes.py:1007-1011` to store scalar `membership_id` (grep: only writer+logout-pop, no other consumers). `_active_context` proof helper reads the real HTTP session via `client.session_transaction()` and calls production `validate_context` + `resolve_effective_permissions` (redis-backing fragility documented in `tests/test_stage4b8_context_switch_e2e_proof.py`). Verification run: Stage 4B core suites (org_admin_remediation_http + context_switch_e2e_proof + auth_context + org_workspace + onboarding + assign_revoke + permission_read_path) → 105 passed; Stage 4B extension suites (4b1 + 4b3 + 4b6 + organisation_role_provisioning + org9 + org10 + host_production_path) → 96 passed; combined 201 passed / 0 failed (target baseline 116 exceeded). GATE: startup `create_app` → `STARTUP_OK`, endpoint validator all-pass; `flask db current`=a91ce8028ae4; `flask db heads`=69cd2e2f7251 (single head; head delta is concurrent transport-lane migration 69cd2e2f7251, untracked, NOT this node - org-admin remediation required no schema change).
- **Links:** tests/test_org_admin_remediation_http.py, tests/conftest.py:423-438, app/identity/routes.py, templates/org/*.html, app/auth/seed_roles.py:216-217, app/identity/ORG_ADMINISTRATION_SPEC.md, BACKLOG.md (Org RBAC audit entry above)

---

## Stage 5 TASK1/TASK2 - Accommodation<->Transport assignment confirmation (FINAL BEHAVIOUR DIRECTIVE)
- Status: Implementation code-complete; functional/e2e proof BLOCKED
- Raised: 2026-09-14
- Context: Two-phase assignment UX (select resource -> confirmation modal with full proposed assignment -> confirm -> CSRF POST) implemented for Accommodation->Transport (guest roster) and Transport->Accommodation (booking show pane), server-authoritative revalidation preserved, Event-faithful notifications (in-service customer email only; internal fact via transactional outbox using 6 new EventType constants: guest_transport_assigned/changed/removed + passenger_accommodation_assigned/changed/removed; no schema change, no NotificationPolicy change -> no double-send), forensic audit stages both directions, detailed assignment success flash from assignment_info, return-journey booking shortcut untouched. Reused NotificationType values only (assign -> booking_confirmed, removed -> booking_update); no enum expansion.
- What needs to happen: Run functional verification once the test DB is unblocked (afcon360_test empty; setup_test_db_schema.py hangs on Redis retry; provider_offering_supply EXCLUDE constraint blocks create_all) — verify confirm-modal flows, rejections (foreign/stale/duplicate ref), notifications, unassign. No new migrations required unless CHECK-constraint sync needed (none added).
- Owner/area: accommodation / transport coordination (Stage 5)
- Links: app/accommodation/services/transport_coordination.py, app/transport/services/accommodation_coordination.py, app/accommodation/routes.py, app/transport/routes.py, templates/accommodation/guest/guest_roster.html, static/js/modules/accommodation/guest_roster.js, templates/transport/bookings/show.html, static/js/modules/transport/accommodation_pane.js, app/notifications/events/registry.py

## Missing Jinja filter datetimeformat blocks ALL transport page rendering (PRE-EXISTING)
- Status: Needs review
- Raised: 2026-09-14 (discovered while verifying Stage 5 TASK2 confirm modal on transport/bookings/show.html)
- Context: `templates/transport/**` uses `|datetimeformat` in 14 places (show.html x9, index.html, incidents/index.html, bookings show) but NO Python file registers this filter (grep: only `strftime` in app.py:55 and `format_number` in app/__init__.py:1654 exist). Jinja raises TemplateAssertionError "No filter named 'datetimeformat'" at compile time, so those pages 500. Present since initial commit (git log -S). Unrelated to Stage 5 (the render failure predates and is independent of the confirm modal addition).
